LYRA Core version 2.3.0 is now available from:
...
[
  {
    ...
    winner: {
        address: addr,    (string) LYRA MN Address,
...
LYRA Core now enables bloom filters by default to support SPV clients like mobile wallets. This feature can be disabled by using the `-peerbloomfilters` option on startup.
...
- #195 `aee05fe` [ZMQ] ZMQ integration for LYRA (Mrs-X)
...
### Tests and QA
- #191 `3a778c3` [Tests] Fix the unit test suite for use with LYRA (Fuzzbawls)
